...
class TestExportersAndModel(unittest.TestCase):
    def setUp(self):
        self.contacts = [
            FacebookUser("1", "Leap", "https://facebook.com/leap", "https://img/leap", 29, 2, None),
            FacebookUser("2", "Known", "https://facebook.com/known", None, 4, 5, 1980),
        ]

    def test_partial_year_is_preserved_in_json(self):
        payload = json.loads(JSONExporter(self.contacts, extracted_at="2025-01-01T00:00:00+00:00").export())
        self.assertIsNone(payload["contacts"][0]["birthday"]["year"])
        self.assertEqual(payload["contacts"][0]["source"], "facebook")
        self.assertEqual(payload["summary"], {"contacts": 2, "with_year": 1, "without_year": 1})
    ...
    def test_sorting_compares_other_day(self):
        self.assertLess(self.contacts[0], FacebookUser("3", "Later", "", "", 1, 3, None))
